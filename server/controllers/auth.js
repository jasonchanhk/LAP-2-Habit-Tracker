...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // since by default the password is not visible in the output, we need to use select +<name of the feild>.
...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      ...
      // Grant access to rendered pages.
      req.user = currentUser;
      return next();
    } catch (err) {
      return next();
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    // roles[("admin", "tech-lead")].role = "user";
